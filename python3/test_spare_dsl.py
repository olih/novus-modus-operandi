...
class TestParsingSpareRow(unittest.TestCase):
    ...
    def test_conversion_should_succeed(self):
        examples = [
            SpareRow().set_id("one").set_v_int(15).set_tags(set(["monday", "tuesday"])).set_emails(["a@email.com", "b@fb.com"]).set_url("http://cc.org").set_color_name_as_str("green").set_tags([SpareItem().set_v_float(1.3).set_v_fraction_as_str("1/4")])
            ]
        for ex in examples:
                spareRow = spareRowParser.parse(ctx, ex.to_nmo_string())
